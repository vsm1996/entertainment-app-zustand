...
export const useEntStore = create<StateProps>((set, get) => ({
  entertainmentData: [
    {
      title: "Beyond Earth",
      thumbnail: {
  ...
  toggleBookmark: (title: string) => {
    set((state) => {
      const updated = state.entertainmentData.map((ent) =>
        ent.title === title ? { ...ent, isBookmarked: !ent.isBookmarked } : ent,
      )
      return { entertainmentData: updated }
    })
  },
  setSearchTerm: (term: string) => set({ searchTerm: term }),
  getFilteredData: (category?: string, bookmarkedOnly?: boolean) => {
    const state = get()
    let filtered = state.entertainmentData

    if (state.searchTerm) {
      filtered = filtered.filter((item) => item.title.toLowerCase().includes(state.searchTerm.toLowerCase()))
    }
  ...
  getTrendingData: () => {
    const state = get()
    return state.getFilteredData().filter((item) => item.isTrending)
  },
  getRecommendedData: () => {
    const state = get()
    return state.getFilteredData().filter((item) => !item.isTrending)
  },
  setHomePage: (page: number) => set({ homePagination: { ...get().homePagination, currentPage: page } }),
  setMoviesPage: (page: number) => set({ moviesPagination: { ...get().moviesPagination, currentPage: page } }),
  setTVSeriesPage: (page: number) => set({ tvSeriesPagination: { ...get().tvSeriesPagination, currentPage: page } }),
  getPaginatedData: (data: ItemProp[], currentPage: number, itemsPerPage: number) => {
